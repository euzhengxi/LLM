...
logging.basicConfig(filename='pipeline_v2_logs.txt', level=logging.INFO)
...
def validate_pddl_planner(domain_filepath:str, pddl_filepath:str):
...
def validate_pddl_llm(problem_description:str, domain_filepath:str, pddl_filepath:str):
    #read files
    ...
    #generate problem description from pddl
    system_prompt = f"You are generating a problem description strictly based on the given pddl and domain pddl. \n\
                      domain_pddl: {domain_pddl}"
    
    user_content = f"This is the problem pddl: {problem_pddl}. \n\
                     Do not add in anything else. Only output the problem description and nothing else."
    ...
    #compare the generated problem description from the actual description
    system_prompt = f"You are comparing 2 problem statements to check if they are semantically similar"

    user_content = f"""Compare the following 2 problem statements. \n\
                    statement1: {problem_description} \n\
                    statement1: {generated_problem} \n\
                    Return ONLY valid JSON, do not include explanations, markdown, code, comments.: {{
                        "match": true/false, \
...
def validate_pddl(domain_filepath:str, pddl_filepath:str, problem_description:str):
    planner_status, planner_error = validate_pddl_planner(domain_filepath=domain_filepath, pddl_filepath=pddl_filepath)
    llm_status, llm_error = validate_pddl_llm(domain_filepath=domain_filepath, pddl_filepath=pddl_filepath, problem_description=problem_description)

    return planner_status and llm_status, f"planner error: [{planner_error}], problem translation mismatch: [{llm_error}]"
...
def generate_diagnosis(problem_description:str, system_prompt:str, invalid_pddls:list, error_logs:list):
    ...
    user_content = f"The following PDDL is wrong. Carefully analyze the planner error and explain the root cause of the error and describe how to fix it. \n \
                    ...
                    Only output the diagnosis and how to solve it step by step"
...
if __name__ == "__main__":
    ...
    for filename, problem_description in problems:
        ...
        for j in range(3):
            isValid, error_log = validate_pddl(domain_filepath=domain_filepath, pddl_filepath=pddl_filepath, problem_description=problem_description)
